...
Stochastic gradient descent is written out in a different but a similar way:

$$ \textit{cost}(\theta, (x^{(i)}, y^{(i)})) = \frac{1}{2} (h_{\theta} (x^{(i)} - y^{(i)})^{2}) $$

The only difference in the above cost function is the elimination of the $m$ constant within $frac{1}{2}$.

$$ J_{train}(\theta) = \frac{1}{m} \sum_{i = 1}^{m} \textit{cost}(\theta, (x^{(i)}, y^{(i)})) $$

$J_{train}$ is now just the aerage of the cost applied to all of our training examples.
...
$$ \Theta_{j} = \Theta_{j} - \alpha(h_{\theta} (x^{(i)}) - y^{(i)}) \cdot x_{j}^{(i)} $$
...
One strategy for trying to actually converge at the global minimum is to **slowly decrease** $\mathbf{\\lpha}$ **over time**. For example $\alpha = \frac{\textit{const}1}{\textit{iterationNumber} + \textit{const}2}$
...
You can update $\Theta$ for each individual $(x, y)$ pair as you collect them. This way, you can adapt to new pools of users, since you are continuously updating theta.
...
$$ \sum_{i = p}^{q} (h_{\theta} (x^{(i)}) - y^{(i)}) \cdot x_{j}^{(i)} $$
...
$$ \Theta_{j} := \Theta_{j} - \alpha \frac{1}{z} (\texit{temp}_{j}^{(1)} + \textit{temp}_{j}^{(2)} + \ldots + \textit{temp}_{j}^{(z)}) $$
